...
RSpec.describe CpmSolver::Solvers::FloydWarshall do
  ...
  context "with a simple two-activity graph" do
    let(:activity_d) { CpmSolver::Core::Activity.new("D", "Task D", 2) }
    let(:activity_e) { CpmSolver::Core::Activity.new("E", "Task E", 3) }

    before do
      # Clear the existing program activities
      program.activities.clear

      # Add only D and E activities
      program.add_activity(activity_d)
      program.add_activity(activity_e)
      # D is the start activity, E depends on D
      program.add_predecessors(activity_e, [activity_d])
    end

    it "calculates early and late times correctly" do
      solver.send(:calculate_early_times)
      solver.send(:calculate_late_times)

      expect(activity_d.early_start).to eq(0)
      expect(activity_d.early_finish).to eq(2)
      expect(activity_e.early_start).to eq(2)
      expect(activity_e.early_finish).to eq(5)

      expect(activity_d.late_start).to eq(0)
      expect(activity_d.late_finish).to eq(2)
      expect(activity_e.late_start).to eq(2)
      expect(activity_e.late_finish).to eq(5)
    end
  end
end
